    ...
    return [
      {
        ...
        headers: [
          ...
          //? script-src/style-src allow 'unsafe-inline': Next.js App Router streams hydration data through per-request inline `<script>self.__next_f.push(...)</script>` tags (content differs per page, so a static hash/nonce can't cover it without opting the whole site into per-request dynamic rendering), and this codebase uses inline `style={{...}}` extensively for CSS custom-property values (hundreds of call sites — style-src has no nonce mechanism at all, only 'unsafe-inline' or per-value hashes). Without this, hydration fails silently in every browser (contact form, ⌘K palette, and the theme toggle all stop responding to input) and most inline-styled visuals (contribution graph, hero glow, gradients, code blocks) don't render. Realistic risk here is low: no user input is ever reflected unescaped into the page (the contact form is a server action, not rendered back).
          {
